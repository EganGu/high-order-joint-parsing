...
from supar.modules.pretrained import TransformerPOSEmbedding
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        if encoder == 'lstm':
            ...
            self.encoder = VariationalLSTM(input_size=n_input,
                                           hidden_size=self.args.n_lstm_hidden//2,
            ...
            self.encoder = TransformerPOSEmbedding(name=self.args.bert,
                                                   n_layers=self.args.n_bert_layers,
                                                   pooling=self.args.bert_pooling,
                                                   pad_index=self.args.pad_index,
                                                   mix_dropout=self.args.mix_dropout,
                                                   finetune=True)
            self.encoder_dropout = nn.Dropout(p=self.args.encoder_dropout)
            self.lstm = VariationalLSTM(input_size=self.encoder.n_out,
                                        hidden_size=self.args.n_lstm_hidden//2,
                                        num_layers=self.args.n_lstm_layers,
                                        bidirectional=True,
                                        dropout=self.args.encoder_dropout)
            self.lstm_dropout = SharedDropout(p=self.args.encoder_dropout)
            # self.args.n_encoder_hidden = self.encoder.n_out
            self.args.n_encoder_hidden = self.args.n_lstm_hidden
            if 'tag' in self.args.feat:
                self.tag_embed = nn.Embedding(num_embeddings=self.args.n_tags,
                                              embedding_dim=self.args.n_encoder_hidden)
                self.encoder.set_POS(self.tag_embed)
        ...
        if embed is not None:
            ...
            if embed.shape[1] != self.args.n_pretrained:
                self.embed_proj = nn.Linear(
                    embed.shape[1], self.args.n_pretrained)
    ...
    def embed(self, words, feats=None):
        ...
        if hasattr(self, 'pretrained'):
            ...
            if self.args.n_embed == self.args.n_pretrained:
                ...
                word_embed = torch.cat(
                    (word_embed, self.embed_proj(pretrained)), -1)
        ...
        if isinstance(self.embed_dropout, IndependentDropout):
            if len(feat_embed) == 0:
                raise RuntimeError(
                    f"`feat` is not allowed to be empty, which is {self.args.feat} now")
            embed = torch.cat(self.embed_dropout(
                word_embed, torch.cat(feat_embed, -1)), -1)
        else:
            embed = word_embed
            if len(feat_embed) > 0:
    ...
    def encode(self, words, feats=None):
        if self.args.encoder == 'lstm':
            x = pack_padded_sequence(self.embed(words, feats), words.ne(
                self.args.pad_index).sum(1).tolist(), True, False)
            x, _ = self.encoder(x)
            x, _ = pad_packed_sequence(x, True, total_length=words.shape[1])
        elif self.args.encoder == 'transformer':
            x = self.encoder(self.embed(words, feats),
                             words.ne(self.args.pad_index))
        else:
            mask = words.ne(self.args.pad_index).any(-1)
            x = pack_padded_sequence(self.encoder_dropout(self.encoder(words, feats=feats)), mask.sum(-1).tolist(), True, False)
            x, _ = self.lstm(x)
            x, _ = pad_packed_sequence(x, True, total_length=mask.shape[1])
            return self.lstm_dropout(x)
            # x = self.encoder(words, feats=feats)
