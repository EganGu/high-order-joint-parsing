...
def kmeans(x: List[int], k: int, max_it: int = 32) -> Tuple[List[float], List[List[int]]]:
    ...
    datapoints, indices, freqs = x.unique(
    ...
    for _ in range(max_it):
        ...
        centroids, old = (datapoints * freqs * mask).sum(-1) / \
            (freqs * mask).sum(-1), centroids
    ...
    clusters = [torch.where(
        indices.unsqueeze(-1).eq(torch.where(y.eq(i))[0]).any(-1))[0].tolist() for i in assigned]
...
def stripe(x: torch.Tensor, n: int, w: int, offset: Tuple = (0, 0), horizontal: bool = True) -> torch.Tensor:
    ...
    return x.as_strided(size=(n, w, *x.shape[2:]),
                        stride=[
                            (seq_len + 1) * numel, (1 if horizontal else seq_len) * numel] + stride[2:],
...
def pad(
    ...
    for i, tensor in enumerate(tensors):
        out_tensor[i][[slice(-i, None) if padding_side ==
                       'left' else slice(0, i) for i in tensor.size()]] = tensor
    return out_tensor
...
def download(url: str, path: Optional[str] = None, reload: bool = False, clean: bool = False) -> str:
    ...
    if not os.path.exists(path):
        ...
        try:
            ...
            raise RuntimeError(
                f"File {url} unavailable. Please try other sources.")
...
def extract(path: str, reload: bool = False, clean: bool = False) -> str:
    ...
    if zipfile.is_zipfile(path):
        with zipfile.ZipFile(path) as f:
            extracted = os.path.join(os.path.dirname(
                path), f.infolist()[0].filename)
