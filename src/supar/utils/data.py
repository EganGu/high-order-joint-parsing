        ...
        **kwargs
        ...
        if cache:
            ...
            if self.bin is None:
                ...
                self.fbin = os.path.join(
                    self.bin, os.path.split(data)[1]) + '.pt'
...
class Sampler(torch.utils.data.Sampler):
    ...
    def __init__(
        ...
        if distributed:
            ...
            self.n_samples = sum(self.n_batches) // self.n_replicas + \
                int(self.rank < sum(self.n_batches) % self.n_replicas)
        self.epoch = 1
    ...
    def __iter__(self):
        ...
        range_fn = torch.arange if not self.shuffle else lambda x: torch.randperm(
            x, generator=g)
        for i, bucket in enumerate(self.buckets):
            split_sizes = [(len(bucket) - j - 1) // self.n_batches[i] +
                           1 for j in range(self.n_batches[i])]
